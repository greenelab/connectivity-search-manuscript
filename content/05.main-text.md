...
Each row shows a different metric computed for the _CbGpPWpG_ metapath that traverses Compound--binds--Gene--participates--Pathway--participates--Gene,
which was selected for illustrative purposes.
Metrics are computed for degree-groups, which is a specific pair or source degree (in this case, the source compound's count of CbG edges) and target degree (in this case, the target gene's count of GpPW edges).
TODO
