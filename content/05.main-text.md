...
since it includes sequences of edges that traverse a single node (i.e walk) or edge (i.e. trail) multiple times.
When computing network-based features to quantify the relationship between a source and target node,
we would like to exclude traversing duplicate nodes (i.e. paths, not walks nor trails) [@doi:10.15363/thinklab.d134].
