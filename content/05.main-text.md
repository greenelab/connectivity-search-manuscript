...
As part of this research, we developed the `xswap` Python [package](https://github.com/greenelab/xswap) with an optimized C implementation for network permutation [@xswap].
...
But 200 is too few to reliably model the null distribution, given the zero-inflated nature of DWPC distributions.
Therefore, we grouped all permuted DWPCs with the same combination of source and target degrees.
