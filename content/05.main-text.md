...
The decision to stop at length 3 was one of practicality, as length 4 adds XX metapaths.
...
For DWPCs, we used 0.5 for the damping exponent.
...
We stored path count matrices in .npy.
We stored DWPCs matrices on-disk in the SciPy sparse matrix format (includes compression, with a `.sparse.npz` extension) [@doi:10.5281/zenodo.1435834].
This format minimizes the space on disk and IO time for the entire matrix, but requires loading the entire matrix and does not offer access to just subsets.

We generated null DWPC summary statistics for all 2,205 metapaths, and bundled them into HetMat archives by metapath length [@doi:10.5281/zenodo.1435834].
