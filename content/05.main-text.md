...
The DWPC is similar to path count (number of paths between the source and target node along a given metapath), expect that it downweights paths though high degree nodes.
...
Previousely, we had no technique for detecting whether a DWPC value was exceptional.
