...
Since our methods only require weighted counts, not fully enumerated paths, adjacency matrix multiplication presents an alternative approach.
...
For example, the metapath _DrDtCrC_  is categorized as a set of disjoint repeats, while _DtCtDtC_ is categorized as repeats of the form BABA.
Many complex repeat patterns can be represented as patterns of smaller patterns, allowing us to compute DWPC for most metapaths up to length 5 and many of length 6 and beyond without enumerating individual paths.
...
Randomly-inserted non-repeated metanodes (eg. _G_ in _DrDaGaDrD_) require no special treatment, and are included in DWPC with a simple matrix multiplication.
...
The recursive, segmented approach we developed allowed us additionally to implement a caching strategy to avoid duplicate DWPC computations.
...
Under this notation, $\mathrm{D}(\mathit{XyZ})$ is the weighted (bi)adjacency matrix for metaedge _XyZ_.
Additionally, let $\mathrm{diag}(A)$ represent a diagonal matrix whose entries are the diagonal elements of $A$.

For the case of short (< 4) repeats for a single metanode, _XaXbX_ (eg. _GiGdG_), we simply subtract the main diagonal.

$$\mathrm{D}(\mathit{XaXbX}) = \mathrm{D}(\mathit{XaX}) \mathrm{D}(\mathit{XbX}) - \mathrm{diag}(\mathrm{D}(\mathit{XaX}) \mathrm{D}(\mathit{XbX}))$$

Nested repeats _XaYbYcX_ (eg. _CtDrDtC_), are treated recursively, with both inner (YY) and outer (XX) repeats treated as short repeats, separately.
...
Overlapping repeats _XaYbXcY_ (eg. _CtDtCtD_) require several corrections ($\odot$ denotes the Hadamard product).
...
The general method is important for patterns such as long ($\geq$ 4) repeats, or complex repeat patterns (eg. of the form ABCABC), but it requires path enumeration and is therefore slower.
As an alternative approach for complex paths, we developed an approximate DWPC method that corrects repeats in disjoint simple patterns but only corrects the first repeat in complex patterns (eg. $\geq$ length four repeat).
